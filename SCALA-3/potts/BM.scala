...
// As of Breeze 2, import this if you want consistent behavior with the same random numbers from execution to execution (modulo threading or other sources of nondeterminacy)
  ...
  object MCMC {
    ...
    def readhOrPi(lines: collection.Iterator[String], nUnits: Int, nStatesOfUnit: Int, whichhOrPi: String ) = {
        ...
        @annotation.tailrec
        def processhPiLine(lines: collection.Iterator[String], h: Array[DenseVector[Double]], n: Int): Tuple2[Int, collection.Iterator[String]] = {
                  if(lines.hasNext) {
                    val line = lines.next()
    ...
    def readJOrPij(lines: collection.Iterator[String], nUnits: Int, nStatesOfUnit: Int, whichJOrPij: String ) = {
        ...
        @annotation.tailrec
        def processJPijLine(lines: collection.Iterator[String], J: Array[DenseMatrix[Double]], n: Int): Tuple2[Int, collection.Iterator[String]] = {
                  if(lines.hasNext) {
                    val line = lines.next()
        ...
        printJOrPij(outJOrPij, stateOrderString, 
                JOrPij.map(pij => {
                        //DenseMatrix( pij: _*)         // for 2.11
                          new DenseMatrix( pij(0).size, pij.size, pij.flatten).t        // for 2.13
                } ), 
                whichJOrPij )
    }
  ...
  object BM {
    ...
    */
    ...
    def learningByRAdam(bmState: BMState, grad: BMInteractions, minOrMax: Int = 1 ):
        ...
        //RAdam         arXiv:1908.03265
          ...
          ( createBMState( bmInteractions = BMInteractions(phia, phijab),
                ...
                learningRates = learningRates,
                eps = eps, step = step),
                optionLearningRates     )       // RAdam cannot be used for the soft thresholding function for L1.
    }

    def learningByModAdamMax(bmState: BMState, grad: BMInteractions, minOrMax: Int = 1 ):
                                                        Tuple2[BMState, Option[BMInteractions]] = {
    ...
    def learningByModAdamSum(bmState: BMState, grad: BMInteractions, minOrMax: Int = 1 ):
                                                        Tuple2[BMState, Option[BMInteractions]] = {
    ...
    def learningByModRAdamMax(bmState: BMState, grad: BMInteractions, minOrMax: Int = 1 ):
                                                        Tuple2[BMState, Option[BMInteractions]] = {
        //RAdam         arXiv:1908.03265
    ...
    def initializeBMStateForRAdam( bmInteractions: BMInteractions, learningRate: Double,
        betaV: Double = 0.999, betaM: Double = 0.9, eps: Double = 1.0e-8, step: Int = 0 ) = {

        initializeBMStateForAdam( bmInteractions, learningRate, betaV, betaM, eps, step )
    }

    def initializeBMStateForModAdamMax( bmInteractions: BMInteractions, learningRate: Double,
        betaV: Double = 0.999, betaM: Double = 0.9, eps: Double = 1.0e-8, step: Int = 0 ) = {

        initializeBMStateForAdam( bmInteractions, learningRate, betaV, betaM, eps, step )
    }

    def initializeBMStateForModAdamSum( bmInteractions: BMInteractions, learningRate: Double,
        betaV: Double = 0.999, betaM: Double = 0.9, eps: Double = 1.0e-7, step: Int = 0 ) = {

        initializeBMStateForModAdamMax( bmInteractions, learningRate, betaV, betaM, eps, step )
    }

    def initializeBMStateForModRAdamMax( bmInteractions: BMInteractions, learningRate: Double,
        betaV: Double = 0.999, betaM: Double = 0.9, eps: Double = 1.0e-8, step: Int = 0 ) = {

        initializeBMStateForRAdam( bmInteractions, learningRate, betaV, betaM, eps, step )
    }
    ...
    def initialize(fia: Array[DenseVector[Double]],
        ...
        val bmState = 
            ...
            else if ( optMethod == "ModRAdamMax" )
                initializeBMStateForModRAdamMax(bmInteractions, learningRate, betaV, betaM, eps, step )
            else if ( optMethod == "ModAdamMax" )
                initializeBMStateForModAdamMax(bmInteractions, learningRate, betaV, betaM, eps, step )
            else if ( optMethod == "ModAdamSum" )
                initializeBMStateForModAdamSum(bmInteractions, learningRate, betaV, betaM, eps, step )
    ...
    def runBM1Step( ioFiles: HashMap[String, File],
        ...
        val mcmc = new MCMC(ioFiles, stateOrderString, interactions, proposedPia = fia )
        ...
        val nextBMStateWithoutSoftPlus =
                if ( optMethod == "NAG" )
                        ...
                        BM.learningByModRAdamMax(bmState, grad )
                else if ( optMethod == "ModAdamMax" ) 
                        BM.learningByModAdamMax(bmState, grad )
                else if ( optMethod == "ModAdamSum" ) 
                        BM.learningByModAdamSum(bmState, grad )
                else if ( optMethod == "Adam" ) 
                        BM.learningByAdam(bmState, grad )
                else if ( optMethod == "RAdam" ) 
                        BM.learningByRAdam(bmState, grad )
                else if ( optMethod == "Adadelta" ) 
  ...
  //val learningrate: Double,
    ...
    def this( ioFiles: HashMap[String, File],
        ...
        betaV: Double, betaM: Double, eps: Double ) = {
                this( ioFiles, stateOrderString, fia.map(fi => DenseVector(fi)),
    ...
    def runBM(
              ...
              ////bmState: BMState, 
                ...
                /*
                gradientDescentMethod: String = "ModAdamSum",
      ...
      //(valueKL1, valueKL2, step, fileInteractions, fileIndpendentMCsamplings)
        ...
        def  iterateBMSteps(
          ...
          val bestKLsRevised =
            if ( bmState.step % logInterval == 0 ) {
                val logFileInteractions = new File(ioFiles("outputDir"), s"log_Interactions_hJ_${bmState.step}.txt")
                val logFileIndpendentMCsamplings = new File(ioFiles("outputDir"), s"log_MC_samples_${bmState.step}.fasta")
          ...
          val fileInteractions = new File(ioFiles("outputDir"), s"Interactions_hJ_${bmState.step}.txt")
          val fileIndpendentMCsamplings = new File(ioFiles("outputDir"), s"MC_samples_${bmState.step}.fasta")
          ...
          val minLearningSteps_rev = if(bmState.step >= minLearningSteps) {
                if ( ioFiles.contains("jobControlParamFile") && ioFiles("jobControlParamFile").exists() ) {
                        ...
                        if( param.contains("minLearningSteps") ) {
          ...
          if(bmState.step >= math.max(minLearningStepsForBestKL, minLearningSteps_rev) && nSamples >= maxNSamples && bmState.step - newBestKL(0).get._3 >= maxNoLearnings ) {
            ...
            iterateBMSteps(runBM1Step, newInteractions, lastConfigurations, 
                initialNSamples, incrementSamples, maxNSamples,
                newBMState, newBestKLs, newBestKL, minLearningSteps_rev)
          }
        ...
        val results  = if ( nSamples == 0 ) {
                          ...
                          initialNSamples, incrementSamples, maxNSamples,
                          newBMState, bestKLs, bestKL, minLearningSteps)
                       } else {
                          iterateBMSteps(runBM1Step, newInteractions, initialConfigurations,
